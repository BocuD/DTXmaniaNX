...
internal class CActPerfGuitarGauge : CActPerfCommonGauge
{
    ...
    public override int OnUpdateAndDraw()
    {
        ...
        {
            txフレーム.Guitar.tDraw2D(CDTXMania.app.Device, n本体X.Guitar, 0, new RectangleF(0, 0, txフレーム.Guitar.Width, 68));
            ...
            txハイスピ.tDraw2D(CDTXMania.app.Device, - 36 + n本体X.Guitar + txフレーム.Guitar.Width, 30, new RectangleF(0, speedTexturePosY, 42, 48));
            if (db現在のゲージ値.Guitar == 1.0 && txフルゲージ != null)
            {
                txフルゲージ.tDraw2D(CDTXMania.app.Device, 6 + n本体X.Guitar, 31, new RectangleF(0, 0, txフレーム.Guitar.Width - 48, 30));
            }
            else if (db現在のゲージ値.Guitar >= 0.0)
            {
                //todo: what the hell is vcscaleratio
                //txゲージ.vcScaleRatio.X = (float)db現在のゲージ値.Guitar;
                txゲージ.tDraw2D(CDTXMania.app.Device, 6 + n本体X.Guitar, 31, new RectangleF(0, 0, txフレーム.Guitar.Width - 48, 30));
            }
            txフレーム.Guitar.tDraw2D(CDTXMania.app.Device, n本体X.Guitar, 0, new RectangleF(0, 68, txフレーム.Guitar.Width, 68));
            /*
        ...
        {
            txフレーム.Bass.tDraw2D(CDTXMania.app.Device, n本体X.Bass - txフレーム.Bass.Width, 0, new RectangleF(0, 0, txフレーム.Bass.Width, 68));
            ...
            txハイスピ.tDraw2D(CDTXMania.app.Device, 4 + n本体X.Bass - txフレーム.Bass.Width, 30, new RectangleF(0, speedTexturePosY, 42, 48));
            if (db現在のゲージ値.Bass == 1.0 && txフルゲージ != null)
            {
                txフルゲージ.tDraw2D(CDTXMania.app.Device, 42 + n本体X.Bass - txフレーム.Bass.Width, 31, new RectangleF(0, 0, txフレーム.Bass.Width - 48, 30));
            }
            else if (db現在のゲージ値.Bass >= 0.0)
            {
                //todo: what the fuck is vsscaleratio.x
                //txゲージ.vcScaleRatio.X = (float)db現在のゲージ値.Bass;
                txゲージ.tDraw2D(CDTXMania.app.Device, 42 + n本体X.Bass - txフレーム.Bass.Width, 31, new RectangleF(0, 0, txフレーム.Bass.Width - 48, 30));
            }
            txフレーム.Bass.tDraw2D(CDTXMania.app.Device, n本体X.Bass - txフレーム.Bass.Width, 0, new RectangleF(0, 68, txフレーム.Bass.Width, 68));
            /*
