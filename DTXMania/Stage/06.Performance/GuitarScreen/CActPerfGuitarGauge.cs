...
internal class CActPerfGuitarGauge : CActPerfCommonGauge
{
    ...
    public override int OnUpdateAndDraw()
    {
        ...
        if (txフレーム.Guitar != null && CDTXMania.DTX.bHasChips.Guitar)
        {
            ...
            txハイスピ.tDraw2D(- 36 + n本体X.Guitar + txフレーム.Guitar.Width, 30, new RectangleF(0, speedTexturePosY, 42, 48), Color4.White, new Vector2(32, 32));
            ...
            {
                var xSize = (float)db現在のゲージ値.Guitar * (txフレーム.Guitar.Width - 48);
                txゲージ.tDraw2D(6 + n本体X.Guitar, 31, new RectangleF(0, 0, txフレーム.Guitar.Width - 48, 30), Color4.White, new Vector2(xSize, 30));
            }
        ...
        if (txフレーム.Bass != null && CDTXMania.DTX.bHasChips.Bass)
        {
            ...
            txハイスピ.tDraw2D(4 + n本体X.Bass - txフレーム.Bass.Width, 30, new RectangleF(0, speedTexturePosY, 42, 48), Color4.White, new Vector2(32, 32));
            ...
            {
                var xSize = (float)db現在のゲージ値.Bass * (txフレーム.Bass.Width - 48);
                txゲージ.tDraw2D(42 + n本体X.Bass - txフレーム.Bass.Width, 31, new RectangleF(0, 0, txフレーム.Bass.Width - 48, 30), Color4.White, new Vector2(xSize, 30));
            }
