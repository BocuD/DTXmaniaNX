    ...
    internal class CDTX : CActivity
    {
        ...
        public class CWAV : IDisposable
        {
            ...
            public override string ToString()
            {
                ...
                sb.Append(string.Format("音量:{0}, 位置:{1}, サイズ:{2}, BGM:{3}, File:{4}, Comment:{5}", n音量, n位置, nチップサイズ,
        ...
        private static void tミラーチップのチャンネルを指定する(CChip chip, EChannel nミラー化前チャンネル番号)
        {
            switch (nミラー化前チャンネル番号)
            {
                ...
                case EChannel.HiHatOpen:
                    ...
                    {
                        chip.nChannelNumber = ((CDTXMania.ConfigIni.eNumOfLanes.Drums == DTXMania.EType.A)
                            ? EChannel.RideCymbal
                            : EChannel.Cymbal);
                        return;
                    }
                    ...
                    break;
                case EChannel.Snare:
                    chip.nChannelNumber = ((CDTXMania.ConfigIni.eNumOfLanes.Drums == DTXMania.EType.C)
                        ? EChannel.LowTom
                        : EChannel.FloorTom);
                    return;
                case EChannel.BassDrum:
                                    ...
                                    case ERandomMode.ANOTHERRANDOM:
                                        ...
                                        else
                                        {
                                            ...
                                            else
                                            {
                                                ...
                                                {
                                                    if (CDTXMania.ConfigIni.eNumOfLanes.Drums == DTXMania.EType.A &&
            ...
            if (part == EInstrumentPart.DRUMS && eRandomPedal == ERandomMode.MIRROR)
            {
                foreach (CChip current in listChip)
                {
                                                          ...
                                                          nチャンネル番号 == EChannel.LeftBassDrum))
                    {
                     ...
                     CDTXMania.ConfigIni.eNumOfLanes.Drums != DTXMania.EType.C)
            {
