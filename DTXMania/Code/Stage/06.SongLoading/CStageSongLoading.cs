...
internal class CStageSongLoading : CStage
{
    ...
    public override void OnManagedCreateResources()
    {
        if (!bNotActivated)
        {
            ...
            try
            {
                ...
                {
                    pfタイトル = new CPrivateFastFont(new FontFamily(CDTXMania.ConfigIni.str選曲リストフォント), 40,
                        FontStyle.Regular);
                    Bitmap bmpSongName = new(1, 1);
                    bmpSongName = pfタイトル.DrawPrivateFont(strSongTitle, CPrivateFont.DrawMode.Edge, Color.Black,
                ...
                {
                    pfアーティスト = new CPrivateFastFont(new FontFamily(CDTXMania.ConfigIni.str選曲リストフォント), 30,
                        FontStyle.Regular);
                    Bitmap bmpArtistName = new(1, 1);
                    bmpArtistName = pfアーティスト.DrawPrivateFont(strArtistName, CPrivateFont.DrawMode.Edge, Color.Black,
                        Color.Black, clGITADORAgradationTopColor, clGITADORAgradationBottomColor, true);
    ...
    public override int OnUpdateAndDraw()
    {
        ...
        switch (ePhaseID)
        {
            ...
            case EPhase.NOWLOADING_DTX_FILE_READING:
            {
                ...
                CScoreIni ini = new(str + ".score.ini");
                ini.tCheckIntegrity();

                if ((CDTXMania.DTX != null) && CDTXMania.DTX.bActivated)
                    CDTXMania.DTX.OnDeactivate();

                CDTXMania.DTX = new CDTX(str, false, ((double)CDTXMania.ConfigIni.nPlaySpeed) / 20.0,
                ...
                foreach (CChip chip in CDTXMania.DTX.listChip)
                {
                    if (chip.eInstrumentPart != EInstrumentPart.UNKNOWN)
                    {
                        ...
                        if (CDTXMania.listTargetGhsotLag[(int)chip.eInstrumentPart] != null)
                        {
                            ...
                            lag.nJudgeTime = chip.nPlaybackTimeMs +
                ...
                if (CDTXMania.bCompactMode)
                    ...
                    CDTXMania.DTX.MIDIレベル =
                        (CDTXMania.stageSongSelection.rConfirmedSong.eNodeType == CSongListNode.ENodeType.SCORE_MIDI)
                            ? CDTXMania.stageSongSelection.nSelectedSongDifficultyLevel
                            : 0;
                ...
                for (int i = 0; i < strSingleLine.Length; i++)
                {
                    ...
                    switch (strA[0])
                    {
                        case "Score":
                            ...
                            continue;
                        case "PlaySkill":
                            CDTXMania.listTargetGhostScoreData[(int)nCurrentInst].dbPerformanceSkill =
                                Convert.ToDouble(strA[1]);
                            continue;
                        case "Skill":
                            CDTXMania.listTargetGhostScoreData[(int)nCurrentInst].dbGameSkill =
                                Convert.ToDouble(strA[1]);
                            continue;
                        case "Perfect":
                            CDTXMania.listTargetGhostScoreData[(int)nCurrentInst].nPerfectCount_ExclAuto =
                                Convert.ToInt32(strA[1]);
                            continue;
                        case "Great":
                            CDTXMania.listTargetGhostScoreData[(int)nCurrentInst].nGreatCount_ExclAuto =
                                Convert.ToInt32(strA[1]);
                            continue;
                        case "Good":
                            CDTXMania.listTargetGhostScoreData[(int)nCurrentInst].nGoodCount_ExclAuto =
                                Convert.ToInt32(strA[1]);
                            continue;
                        case "Poor":
                            CDTXMania.listTargetGhostScoreData[(int)nCurrentInst].nPoorCount_ExclAuto =
                                Convert.ToInt32(strA[1]);
                            continue;
                        case "Miss":
                            CDTXMania.listTargetGhostScoreData[(int)nCurrentInst].nMissCount_ExclAuto =
                                Convert.ToInt32(strA[1]);
                            continue;
                        case "MaxCombo":
    ...
    private void tDrawStringLarge(int x, int y, string str, bool bExtraLarge = false)
    {
