...
// Declarative schema for Config.ini.
//
// Each configuration value is described exactly once by a ConfigItem that knows how to both
// read itself from and write itself to the INI text. tWrite() and tReadFromString() are driven
// entirely by this schema, so there is a single source of truth per setting: adding a new value
// means adding one line here rather than editing a giant write method and a giant parse method.
//
// The format remains fully backwards compatible: only the section- and key-names matter when
// reading, and the reader tolerates missing, reordered, or unknown entries. Comments/descriptions
// live next to the value they describe and are emitted verbatim on write.
internal partial class CConfigIni
{
	#region [ schema model ]

	/// <summary>
	/// A single INI key. Knows how to parse its value from a string and (optionally) how to render
	/// its value back to a string. A null <see cref="Write"/> makes the item read-only: it is still
	/// accepted when parsing (for backwards compatibility) but is never written out.
	/// </summary>
	private sealed class ConfigItem
	{
		...
		public ConfigItem(string key, Action<CConfigIni, string> read, Func<CConfigIni, string> write = null)
		{
			Key = key;
			Read = read;
			Write = write;
	...
	// The declarative description of every key=value section of Config.ini. Sections whose contents
	// are not simple key=value pairs ([AutoPlay], [HitRange], [GUID], the key-assign sections) are
	// still handled by dedicated code in tReadFromString/tWrite.
	// Verbatim documentation block written just before the [DrumsKeyAssign] section. Declared before
	// Schema so it is initialised first (static field initialisers run in textual order).
	...
	// effect when it parses within range, mirroring the original tCompose-based behaviour.
	...
	private static string tWriteSkinPath(CConfigIni c)
	{
		...
		Uri uriPath = new Uri(Path.Combine(c.strSystemSkinSubfolderFullName, "." + Path.DirectorySeparatorChar));
	...
	private static void tReadSkinPath(CConfigIni c, string v)
	{
		...
		if (!Path.IsPathRooted(v))
		{
			...
			absSkinPath = u.AbsolutePath.ToString();                      // v内に相対パスがある場合に備える
	...
	/// <summary>Whether the given section name is described by <see cref="Schema"/>.</summary>
	private static bool bIsSchemaSection(string sectionName) => SchemaByKey.ContainsKey(sectionName);

	/// <summary>
	/// Applies a single "key=value" line belonging to a schema section. Unknown keys are ignored,
	/// matching the reader's tolerance for missing/renamed entries.
	/// </summary>
