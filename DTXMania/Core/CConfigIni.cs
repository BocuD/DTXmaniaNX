...
//          this.nASIOBufferSizeMs = 0;                 // #24820 2012.12.25 yyagi 初期値は0(自動設定)
	...
	public void tWrite( string iniFilename )
	{
		...
		// [GUID] uses repeated JoystickID keys, so it is not a simple key=value section.
		sw.WriteLine( "[GUID]" );
		sw.WriteLine();
	...
	private void tReadFromString( string strAllSettings )	// 2011.4.13 yyagi; refactored to make initial KeyConfig easier.
	{
		...
		foreach ( string s in strSingleLine )
		{
			...
			try
			{
				...
				// Almost every section is described declaratively by the schema (CConfigIni.Schema.cs)
				// and dispatched through a single lookup. [GUID] is the only exception: it uses repeated
				// JoystickID keys, so it cannot be modelled as one key -> one value.
				if ( bIsSchemaSection( currentSectionName ) )
				{
					tReadSchemaField( currentSectionName, str3, str4 );
				}
				else if ( currentSectionName.Equals( "GUID" ) && str3.Equals( "JoystickID" ) )
				{
