namespace FDK
{
    public class CConversion // C変換
    {
        ...
        public static readonly string strBase36Characters =
        ...
        public static int
            nGetNumberIfInRange(string strNumber, int nMin, int nMax,
                int nDefaultValueOnFailure) // n値を文字列から取得して範囲内に丸めて返す
        {
            int num;
        ...
        public static int
            nRoundToRange(string str数値文字列, int n最小値, int n最大値, int n取得失敗時のデフォルト値) // n値を文字列から取得して範囲内にちゃんと丸めて返す
        {
        ...
        public static int nConvert2DigitHexadecimalStringToNumber(string strNum, int offset = 0) // n16進数2桁の文字列を数値に変換して返す
        {
            if (strNum.Length < 2)
                return -1;

            int digit2 = strBase16Characters.IndexOf(strNum[offset]);
            if (digit2 < 0)
                return -1;
            ...
            int digit1 = strBase16Characters.IndexOf(strNum[offset + 1]);
            if (digit1 < 0)
                return -1;
        ...
        public static int nConvert2DigitBase36StringToNumber(string strNum, int offset = 0) // n36進数2桁の文字列を数値に変換して返す
        {
            if (strNum.Length < 2)
                return -1;

            int digit2 = strBase36Characters.IndexOf(strNum[offset]);
            if (digit2 < 0)
                return -1;
            ...
            int digit1 = strBase36Characters.IndexOf(strNum[offset + 1]);
            if (digit1 < 0)
                return -1;
