...
public class CSoundDeviceWASAPI : ISoundDevice
{
	...
	public CSoundDeviceWASAPI(Eデバイスモード mode, long n希望バッファサイズms, long n更新間隔ms, string strRecordFileType, string strEncoderPath)
	{
		...
		tWasapiProc = new WASAPIPROC(tWASAPI処理);      // アンマネージに渡す delegate は、フィールドとして保持しておかないとGCでアドレスが変わってしまう。
		...
		if (nDevNo != -1)
		{
			...
			if (!Bass.BASS_Init(0, nFrequency, BASSInit.BASS_DEVICE_DEFAULT, IntPtr.Zero))
			{
				...
				{
					Trace.TraceError("BASS_Init failed (WASAPI {0}). Error: {1}", mode.ToString(), err.ToString());
					throw new Exception(string.Format("BASS (WASAPI {0}) initialization failed. (BASS_Init)[{1}]", mode.ToString(), err.ToString()));
				}
				else
				{
		...
		Trace.TraceInformation("Requested Buffer Size (ms): " + n希望バッファサイズms);
		Trace.TraceInformation("Requested Update Period (ms): " + n更新間隔ms);
		Trace.TraceInformation("fPeriod = " + fPeriod + " (Exclusive: minperiod, Shared: defperiod. Before Win10 low latency audio considerations)");
		...
		Trace.TraceInformation("f希望バッファサイズsec=" + f希望バッファサイズsec + ", f更新間隔sec=" + f更新間隔sec + ": After Win10 low latency audio considerations");
		...
		if (bSuccess)
		{
			if (mode == Eデバイスモード.排他)
			{
				...
				switch (wasapiInfo.format)      // BASS WASAPI always uses 32bit float for internal processing, but the device might not.
				{
				...
				Trace.TraceInformation("Device Min Update Period: {0}ms, Default Update Period: {1}ms", deviceInfo.minperiod * 1000, deviceInfo.defperiod * 1000);
				bIsBASSFree = false;
				//-----------------
				#endregion
			}
			else
			{
				...
				Trace.TraceInformation("Using Device: #{0} : {1}, flags={2}", nDevNo, deviceInfo.name, deviceInfo.flags);
				Trace.TraceInformation("BASS WASAPI Initialized (Shared Mode, {0}Hz, {1}ch, Format: {2}, Buffer: {3} bytes [{4}ms (Requested: {5}ms)], Update Period: {6}ms)",
					wasapiInfo.freq,
					wasapiInfo.chans,
					wasapiInfo.format.ToString(),
					wasapiInfo.buflen,
		...
		else if (mode == Eデバイスモード.排他)
		{
			...
			Trace.TraceError("BASS_WASAPI_Init failed (WASAPI {0}). Error: {1}", mode.ToString(), errcode);
			Bass.BASS_Free();
			bIsBASSFree = true;
			throw new Exception(string.Format("BASS (WASAPI {0}) initialization failed. (BASS_WASAPI_Init)[{1}]", mode.ToString(), errcode));
		}
		else
		{
			BASSError errcode = Bass.BASS_ErrorGetCode();
...
#else
		hMixer = BassMix.BASS_Mixer_StreamCreate(
			info.freq,
			info.chans,
		...
		for (int i = 0; i <= (int)CSound.EInstType.Unknown; i++)
		{
...
#else
			hMixer_Chips[i] = BassMix.BASS_Mixer_StreamCreate(
				info.freq,
				info.chans,
				BASSFlag.BASS_MIXER_NONSTOP | BASSFlag.BASS_SAMPLE_FLOAT | BASSFlag.BASS_STREAM_DECODE | BASSFlag.BASS_MIXER_POSEX);    // デコードのみ＝発声しない。WASAPIに出力されるだけ。
#endif
			...
			// Mixerのボリューム設定
			Bass.BASS_ChannelSetAttribute(hMixer_Chips[i], BASSAttribute.BASS_ATTRIB_VOL, CSoundManager.nMixerVolume[i] / 100.0f);
...
#else
			bool b1 = BassMix.BASS_Mixer_StreamAddChannel(hMixer, hMixer_Chips[i], BASSFlag.BASS_DEFAULT);
#endif
			if (!b1)
			{
				BASSError errcode = Bass.BASS_ErrorGetCode();
		...
		if (!string.IsNullOrEmpty(strRecordFileType))
		{
			switch (strRecordFileType.ToUpper())
			{
				case "WAV":
				{
					var e = new EncoderWAV(hMixer_DeviceOut);
				...
				case "OGG":
				{
					var e = new EncoderOGG(hMixer_DeviceOut);
				...
				case "MP3":
				{
					var e = new EncoderLAME(hMixer_DeviceOut);
					e.EncoderDirectory = strEncoderPath;
					e.LAME_UseVBR = false;
					...
					break;
				default:
					encoder = new EncoderWAV(hMixer_DeviceOut);
					break;
			}
	...
	protected void Dispose(bool bManagedDispose)
	{
		...
		if (hMixer_Record != 0)
		{
			BassMix.BASS_Mixer_ChannelPause(hMixer_Record);
			Bass.BASS_StreamFree(hMixer_Record);
			hMixer_Record = 0;
		}
		...
		if (hMixer_Chips != null)
		{
			for (int i = 0; i <= (int)CSound.EInstType.Unknown; i++)
			{
				if (hMixer_Chips[i] != 0)
				{
					...
					BassMix.BASS_Mixer_ChannelPause(hMixer_Chips[i]);
					Bass.BASS_StreamFree(hMixer_Chips[i]);
					hMixer_Chips[i] = 0;
				}
			}
		}
	...
	protected int tWASAPI処理(IntPtr buffer, int length, IntPtr user)
	{
		...
		n経過時間ms = (n累積転送バイト数 - n未再生バイト数) * 1000 / nミキサーの1秒あたりのバイト数;
		n経過時間を更新したシステム時刻ms = tmシステムタイマ.nSystemTimeMs;

		// 実出力遅延を更新。
		// 未再生バイト数の平均値。

		long n今回の遅延ms = n未再生バイト数 * 1000 / nミキサーの1秒あたりのバイト数;
		n実出力遅延ms = (b最初の実出力遅延算出) ? n今回の遅延ms : (n実出力遅延ms + n今回の遅延ms) / 2;
		b最初の実出力遅延算出 = false;
